...
string check(string m,string st)
{
        ...
        for(i=0; i<j; i++)
        {
            ...
            l=0; while(st1[l]==48 && l<st1.size()-1) {st1.erase(st1.begin()+0); }
            //cout<<"^"<<st1<<endl;
            k=st1.size();
            if(k>mx) { st2=st1;  mx=k; }//cout<<"#"<<st2<<endl;}
            if(k==mx && st1>st2) { st2=st1; } //cout<<"#"<<st2<<endl;
        }
...
int main()
{
    ...
    while(cin>>st)
    {
     ...
     l=0; while(ans[l]==48 && l<ans.size()-1) {ans.erase(ans.begin()+0);}
     if(ans=="")ans="0";
     cout<<ans<<endl;
    }

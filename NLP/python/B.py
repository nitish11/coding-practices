	...
	for i in range(len(sentences)):
		...
		for m in matchIter:
	        ...
	        if  dates:
		        ...
		        lcomplexDatePattern = re.compile(r"(?P<Date>[0-9]{1,2}) of (?P<Month>(January|February|March|April|May|June|July|August|September|October|November|December|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)), (?P<Year>[0-9]{2,4})",re.IGNORECASE)
		        matchIter = lcomplexDatePattern.finditer(sentences[i])		      
		        for m in matchIter:
				tempDate = str(i) + " " + m.group('Date').zfill(2) + '/' + lmonth[m.group('Month').lower()] + '/' + m.group('Year')
				dates.append(tempDate)

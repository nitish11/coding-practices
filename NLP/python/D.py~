	...
	for i in range(len(sentences)):
		...
		for m in matchIter:
	        ...
	        if dates :
		        ...
		        lcomplexDatePattern = re.compile(r"(?P<Date>[0-9]{1,2})(?P<ex>(st|th|nd|rd)*) (?P<Month>(January|February|March|April|May|June|July|August|September|October|November|December|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec))(,*) (?P<Year>[0-9]{2,4})",re.IGNORECASE)
	        ...
	        if dates :
		        ...
		        lcomplexDatePattern = re.compile(r"(?P<Year>[0-9]{2,4})(?P<ex>(.|-|\/))(?P<Month>[0-9]{1,2})(?P<sp>(.|-|\/))(?P<Date>[0-9]{1,2})")
		        matchIter = lcomplexDatePattern.finditer(sentences[i])		      
		        for m in matchIter:
				tempDate = str(i) + " " + m.group('Date').zfill(2) + '/' + m.group('Month').zfill(2) + '/' + m.group('Year').zfill(4)
				dates.append(tempDate)
	        ...
	        if dates :
		        ...
		        lcomplexDatePattern = re.compile(r"(?P<Month>(January|February|March|April|May|June|July|August|September|October|November|December|Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)) (?P<Date>[0-9]{1,2})(?P<ex>(st|th|nd|rd)*)(,*)(?P<Year>[0-9]{2,4})",re.IGNORECASE)
		        matchIter = lcomplexDatePattern.finditer(sentences[i])		      
		        for m in matchIter:
				tempDate = str(i) + " " + m.group('Date').zfill(2) + '/' + lmonth[m.group('Month').lower()].zfill(2) + '/' + m.group('Year').zfill(4)
				dates.append(tempDate)
...
if __name__ == "__main__":
	dates = extractDates(readMyFile("./data/testinputnew"))
	for date in dates:
		print date

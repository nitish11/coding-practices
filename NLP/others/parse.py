	...
	for i in range(len(sentences)):
		...
		# TODO : CODE HERE
			...
			if  dates:
		                ...
		                complexDatePattern = re.compile(r"(?P<Month>[0-9]{1,2})(?P<ex>(.|-|\/)*)(?P<Date>[0-9]{1,2})(?P<exe>(.|-|\/)*)(?P<Year>[0-9]{2,4})")
			...
			if  dates:
		                ...
		                complexDatePattern = re.compile(r"(?P<Year>[0-9]{2,4})(?P<ex>(.|-|\/)*)(?P<Month>[0-9]{1,2})(?P<exe>(.|-|\/)*)(?P<Date>[0-9]{1,2})")
		                matchIter = complexDatePattern.finditer(sentences[i])
		                for m in matchIter:
				        tempDate = str(i) + " " + m.group('Date').zfill(2) + '/' + m.group('Month').zfill(2) + '/' + m.group('Year').zfill(4)
				        dates.append(tempDate)
